...
///
/// The [Type] can be any type(so lists of data are stored when navigating back and forward).
///
...
final class SimpleChangeListener<Type> extends StatefulWidget {
  /// Rebuilds with the current [data] of the [SimpleChangeStream]
  final Widget Function(BuildContext context, Type data, Widget? child) builder;

  /// The target stream for which this widget will listen to changes of its [SimpleChangeStream.changeValue]
  final SimpleChangeStream<Type> streamToListenTo;
  ...
  @override
  State<SimpleChangeListener<Type>> createState() => _SimpleChangeListenerState<Type>();
}

final class _SimpleChangeListenerState<Type> extends State<SimpleChangeListener<Type>> {
  StreamSubscription<Type>? subscription;
  Type? currentData;

  @override
  Widget build(BuildContext context) {
    return widget.builder.call(context, currentData as Type, widget.child);
  }

  @override
  void initState() {
    super.initState();
    subscription = widget.streamToListenTo.listenToEvents((Type data) {
      setState(() {
        currentData = data;
      });
    });
